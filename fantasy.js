...
export async function fantasyEntryOpen(mode='test'){
 const key=mode==='live'?'fantasy_entry_open':'fantasy_test_entry_open',v=t(await getSetting(key).catch(()=>'' )).toLowerCase();
 return mode!=='closed'&&['1','yes','true','on','open'].includes(v);
}
...
export async function getFantasyBootstrap(id,owner,lang='en',mode='test'){
 ...
 const team=seasonRows.find(x=>String(x.telegram_id)===String(id))||null;
 ...
 return{lang,mode,is_test:mode==='test',entry_open:entryOpen,preview_only:!entryOpen,banner,season:catalog.season,budget:catalog.budget,roster_size:8,max_per_pool:2,transfers:2,lock_at:catalog.lockAt,locked:deadlinePassed(catalog.lockAt),rules:RULES[lang],rules_i18n:RULES,scoring:FANTASY_SCORING,pricing:FANTASY_PRICING,players:fantasyPlayers,player_leaderboard:playerLeaderboard,team:publicTeam(team),leaderboard,owner_name:owner||''};
}
