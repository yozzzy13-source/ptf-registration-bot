...
export const ADMIN_COMMAND_LIST = [
  { cmd:'admin',        group:'Панель и рассылки', short:'Админ-панель',
    help:'админ-панель: игроки, фильтры, рассылки, события (правка и удаление), балансы, ручные возвраты, кнопки меню по группам' },
  ...
  { cmd:'whois',        group:'Настройка', short:'Что бот видит по игроку', args:'@ник или telegram_id',
    help:'узнаётся ли игрок по id, какая у него группа и какие кнопки ему достаются' },
  { cmd:'id_check',     group:'Настройка', short:'Проверка telegram_id',
    help:'у кого в базе нет telegram_id и где дубли — из-за них меню у игрока разное' },
  { cmd:'photos',       group:'Настройка', short:'Откуда берутся аватарки',
    help:'сколько фото читается из Players_Master, витрины и своих аватарок, и у кого фото нет' },
  ...
  { cmd:'profile_refresh', group:'Настройка', short:'Обновить витрину профилей', args:'[go]',
    help:'показать формулы IMPORTRANGE в витрине профилей; с «go» — заставить их пересчитаться сейчас' },
  { cmd:'places', group:'Настройка', short:'Проверить места в дивизионах', args:'[сезон]',
    help:'что бот видит в живых таблицах дивизионов и совпали ли имена с витриной профилей; кого там нет — у того в карточке не будет номера места' },
  { cmd:'result_test',  group:'Матчи', short:'Предпросмотр карточки результата', args:'[имя игрока]',
    help:'показать, как карточка и текст уйдут в ленту и в личку, на настоящем матче; вторым словом можно назвать игрока, иначе берётся последний результат; никому, кроме вас, не отправляется' },
  { cmd:'test_match',   group:'Матчи', short:'Боевой тест результата с откатом', args:'<победитель | проигравший | счёт>',
    help:'прогнать матч по всей цепочке по-настоящему: запись в общий лог и в таблицу дивизиона, сезон, пересчёт мест, снимок формы и очков Fantasy, карточка. В ленту и игрокам ничего не уходит — карточка приходит только вам, а под ней кнопка «Откатить», которая возвращает таблицы в исходное состояние. Счёт пишется от победителя: /test_match Иван | Пётр | 6:4 6:3' },
  { cmd:'fix_result',   group:'Матчи', short:'Перевыпустить карточку результата', args:'<id сообщения> [имя игрока]',
    help:'заменить картинку, текст и кнопки у уже опубликованного результата в ленте; id сообщения — последнее число в ссылке на пост, вторым аргументом можно назвать игрока, иначе берётся последний результат; править можно первые 48 часов' },
