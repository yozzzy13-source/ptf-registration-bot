...
export async function handleMessage(msg) {
  ...
  if (state?.mode === 'awaiting_payment_proof') {
    ...
    await updateApplication(state.applicationId, { application_status:'proof_received', payment_status:'proof_received', payment_proof_status:'proof_received', payment_proof_file_id:fileId });
    await updateApplicantStatusByTelegramId(from.id, 'proof_received');
    await logPayment({ payment_id:state.paymentId, application_id:state.applicationId, telegram_id:from.id, player_name:app.player_name, event_id:app.event_id, event_name:app.event_name, proof_file_id:fileId, proof_received_at:nowISO(), status:'proof_received' });
    userState.delete(String(chatId));
    try {
