...
async function userLang(from) { const saved = await findApplicantByTelegramId(from.id).catch(() => null); return saved?.language || langOf(from.language_code); }
async function sendMain(chatId, lang) { closeContactSession(chatId); userState.delete(String(chatId)); const txt=await getBotText('welcome_main',lang); await sendMessage(chatId, txt?.html_text || '<b>Welcome to Phuket Tennis Family</b> 🎾', {reply_markup:mainKeyboard(lang)}); }
