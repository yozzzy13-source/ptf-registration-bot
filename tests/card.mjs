...
assert.deepEqual(await pixel(png,225,420),[239,52,52],'Applicants portrait takes priority');
assert.deepEqual(await pixel(png,855,420),[52,84,239],'Master photo is second choice');
card.forgetPhotoCache();failAvatar=true;downloads=[];
const fallback=await card.renderMatchCard({...match,loser:'No Portrait',loserId:''});
assert.deepEqual(downloads,['https://portraits.test/master.png']);
assert.deepEqual(await pixel(fallback,225,420),[52,84,239],'Broken Applicants avatar falls back to Master');
