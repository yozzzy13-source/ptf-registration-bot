...
const pairs = [
 ...
 ['Missing broadcast text: RU','Заполните русский текст рассылки: среди получателей есть игроки с языком RU.','Fill in the Russian broadcast text: there are RU recipients.'],
  ...
  // Эти семь сообщений раньше не переводились и превращались в дежурное
  // «не удалось выполнить действие»: человек видел красную полосу и не понимал,
  // что именно поправить. Самое частое — несовпадение победителя со счётом.
  ['Указанный победитель не совпадает со счётом',
   'Победитель не совпадает со счётом: счёт пишется от победителя, поэтому у него должно быть больше выигранных сетов. Поменяйте победителя или переверните счёт.',
   'The winner does not match the score: the score is written from the winner’s side, so they must have more sets won. Switch the winner or flip the score.'],
  ['Выберите победителя или результат для обоих',
   'Выберите победителя или отметьте результат для обоих.',
   'Select the winner, or mark the result for both players.'],
  ['Для RET укажите сыгранный счёт',
   'Для RET укажите счёт, который успели сыграть.',
   'For RET, enter the score that was actually played.'],
  ['Выберите двух разных игроков','Выберите двух разных игроков.','Select two different players.'],
  ['Игроки должны быть из одного дивизиона','Игроки должны быть из одного дивизиона.','Both players must be in the same division.'],
  ['Только для организатора','Только для организатора.','Organiser only.'],
