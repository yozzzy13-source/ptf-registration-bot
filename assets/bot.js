...
async function preparePosterForAdmin({ chatId, threadId='', slot, comment='' }) {
  const [{ preparePosterJob }, { winnerFirstScore }, { archivePosterJob }] = await Promise.all([
    import('./matchposter.js'), import('./matches.js'), import('./matcharchive.js')
  ]);
  ...
  let archive={saved:false,reason:'not_configured'};
  if(rootFolderId) {
    archive=await archivePosterJob(slot,{rootFolderId,season,job})
      .catch(e=>({saved:false,reason:e.message}));
  }
  const opts=threadId?{message_thread_id:threadId}:{};
  const consent=job.consent.map(x=>`${x.allowed?'✅':'⛔'} ${escapeHtml(x.name)}: <b>${escapeHtml(x.value)}</b>`).join('\n');
  const status=job.status==='blocked_consent'
    ? '⛔ Заблокировано: нет согласия обоих игроков. Фотографии не будут переданы во внешний API.'
    : '🟡 Промпты готовы. Image API пока не подключён, изображения не создавались.';
  const drive=archive.saved
    ? `\n<a href="${escapeHtml(archive.job?.url || '')}">Задание на Google Drive</a>`
    : `\nDrive: ${escapeHtml(archive.reason || 'не настроен')}`;
  await sendMessage(chatId,`<b>🎨 Poster job</b>\n\n${status}\n\n<b>Согласия</b>\n${consent}\n\nПромпт: <code>${escapeHtml(job.prompt_source)}</code>\nВариантов: <b>${job.prompts.length}</b>${job.comment?`\nКомментарий: <i>${escapeHtml(job.comment)}</i>`:''}${drive}`,{
    ...opts,
    reply_markup:{inline_keyboard:[
      [{text:'🔄 Подготовить заново',callback_data:`poster:prepare:${job.match_id}`}],
      [{text:'✍️ Новый комментарий',callback_data:`poster:comment:${job.match_id}`}]
    ]}
  });
  for(const item of job.prompts) {
    const raw=String(item.prompt||'');
    const chunks=[];
    for(let i=0;i<raw.length;i+=2800)chunks.push(raw.slice(i,i+2800));
    for(let i=0;i<Math.max(1,chunks.length);i++) {
      await sendMessage(chatId,`<b>Prompt · вариант ${item.variant}${chunks.length>1?` · ${i+1}/${chunks.length}`:''}</b>\n<pre>${escapeHtml(chunks[i]||'')}</pre>`,opts);
    }
  }
  return job;
}
