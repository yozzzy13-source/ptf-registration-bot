...
function filters(){return {status:$('fStatus').value,division:$('fDivision').value,language:$('fLanguage').value,selfie_status:$('fSelfie').value,rating:$('fRating').value,instagram_status:$('fInstagram').value,photo_consent:$('fConsent').value,event:$('fEvent').value,search:$('fSearch').value.trim(),selected_ids:[...selected]}}
function noSelectionFilters(){const f=filters(); delete f.selected_ids; return f}
function matches(c){const f=noSelectionFilters(); const n=v=>String(v||'').toLowerCase(); if(f.status&&n(c.status)!==n(f.status))return false;if(f.division&&n(c.division)!==n(f.division))return false;if(f.language){if(f.language==='ru'&&n(c.language)!=='ru')return false;if(f.language==='en'&&n(c.language)==='ru')return false}if(f.selfie_status){if(f.selfie_status==='missing'){if(n(c.selfie_status)==='received')return false}else if(n(c.selfie_status)!==n(f.selfie_status))return false}if(f.event&&!n(c.last_application_event).includes(n(f.event)))return false;if(f.rating==='missing'&&!c.missing_rating)return false;if(f.rating==='set'&&c.missing_rating)return false;const hasIg=Boolean(String(c.instagram||'').trim()),igState=n(c.instagram_status)||(hasIg?'provided':'');if(f.instagram_status==='provided'&&!hasIg)return false;if(f.instagram_status==='none'&&igState!=='no_account')return false;if(f.instagram_status==='missing'&&(hasIg||igState==='no_account'))return false;const consent=n(c.photo_publication_consent);if(f.photo_consent==='yes'&&consent!=='yes')return false;if(f.photo_consent==='no'&&consent!=='no')return false;if(f.photo_consent==='missing'&&['yes','no'].includes(consent))return false;if(f.search){const hay=[c.name,c.telegram_username,c.telegram_id,c.whatsapp,c.country,c.crm_tags].map(n).join(' ');if(!hay.includes(n(f.search)))return false}return true}
function renderStats(stats){const labels=window.PTF_LANG==='ru'?{contacts:'Контакты',applications:'Заявки',active:'Активные',waitlist:'Лист ожидания',unpaid:'Ожидают оплаты',proofReceived:'Получены чеки',paid:'Оплата подтверждена',rejectedPayments:'Оплата отклонена',paidThb:'Оплачено THB',paidUsdt:'Оплачено USDT',missingSelfie:'Нет селфи',consentYes:'Фото: разрешили',consentNo:'Фото: отказали',consentPending:'Фото: нет ответа'}:{contacts:'Contacts',applications:'Applications',active:'Active',waitlist:'Waitlist',unpaid:'Unpaid / waiting',proofReceived:'Proof received',paid:'Paid approved',rejectedPayments:'Payment rejected',paidThb:'Paid THB',paidUsdt:'Paid USDT',missingSelfie:'Missing selfies',consentYes:'Photo: allowed',consentNo:'Photo: declined',consentPending:'Photo: pending'};$('stats').innerHTML=Object.entries(stats).map(([k,v])=>`<div class="stat"><b>${v}</b><span>${labels[k]||k}</span></div>`).join('')}
...
function renderTable(){filtered=contacts.filter(matches);$('countLine').textContent=`${AUI("Найдено: ")}${filtered.length}${AUI(" · отмечено: ")}${selected.size}`;let html=AUI("<thead><tr><th><input type=\"checkbox\" onchange=\"toggleAll(this.checked)\"></th><th>Игрок</th><th>Статус</th><th>Дивизион</th><th class=\"lo\">NTRP</th><th class=\"lo\">Селфи</th><th class=\"lo\">Instagram</th><th>Фото</th><th class=\"lo\">Язык</th><th class=\"lo\">Ник</th><th class=\"lo\">Событие</th><th class=\"lo\">Страна</th><th class=\"lo\">WhatsApp</th><th></th></tr></thead><tbody>");html+=filtered.map(c=>`<tr><td><input type="checkbox" ${selected.has(String(c.telegram_id))?'checked':''} onchange="toggleOne('${c.telegram_id}',this.checked)"></td><td class="grow"><b>${esc(c.name)}</b><br><span class="muted">${c.telegram_username?'@'+esc(c.telegram_username)+' · ':''}${esc(c.telegram_id)}</span></td><td><span class="pill">${esc(adminValue(c.status))}</span></td><td>${esc(c.division)}</td><td class="lo">${c.missing_rating?'<span class="err">—</span>':esc(c.ntrp)}</td><td class="lo">${esc(adminValue(c.selfie_status))}</td><td class="lo">${c.instagram?esc(c.instagram):(String(c.instagram_status||"").toUpperCase()==="NO_ACCOUNT"?"нет":"—")}</td><td>${String(c.photo_publication_consent||"").toUpperCase()==="YES"?'<span class="ok">Да</span>':String(c.photo_publication_consent||"").toUpperCase()==="NO"?'<span class="err">Нет</span>':'—'}</td><td class="lo">${esc(c.language)}</td><td class="lo">${c.telegram_username?'@'+esc(c.telegram_username):''}</td><td class="lo">${esc(c.last_application_event)}</td><td class="lo">${esc(c.country)}</td><td class="lo">${esc(c.whatsapp)}</td><td><button class="btn secondary" onclick="event.stopPropagation();avOpen('${c.telegram_id}${AUI("')\">Аватар</button>")}${String(c.status||'').toLowerCase()==='active'?'':` <button class="btn secondary" onclick="event.stopPropagation();activatePlayer('${c.telegram_id}${AUI("')\">Активировать</button>")}`}</td></tr>`).join('');html+='</tbody>';$('playersTable').innerHTML=html;renderRecipients()}
...
async function requestSocialData(kind){
  const instagram=kind==='instagram',box=$(instagram?'instagramResult':'consentResult');
  try{
    const label=instagram?'Instagram-аккаунты':'согласие на публикацию фотографий и PTF-аватаров';
    if(!confirm('Отправить отдельный запрос: '+label+'?'))return;
    const useSelected=selected.size>0;
    const j=await api('/api/admin/request-social-data',{kind,filters:useSelected?{selected_ids:[...selected]}:noSelectionFilters()});
    box.innerHTML=`✅ Ушло: <b>${j.sent}</b>, ошибок: <b>${j.failed}</b>, получателей: <b>${j.recipients}</b>`;
    await reload();
  }catch(e){box.innerHTML=`<span class="err">${esc(e.message)}</span>`}
}
function requestInstagramAccounts(){return requestSocialData('instagram')}
function requestPhotoConsent(){return requestSocialData('consent')}
